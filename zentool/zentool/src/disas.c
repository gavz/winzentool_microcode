...
#define print_field(obj, name, fmt, _offset, _size) do {                                            \
    print_struct_field(#name,                                                       \
                       fmt,                                                         \
                       _size,                                                       \
                       (8 * sizeof(*(obj))) - (_offset),                              \
                       (obj)->name);                                                \
} while (false)

#define REGISTER_BITFIELD(_type, _name, _offset, _size) do {                        \
    print_field(&op, _name, "%4X", _offset, _size);                                 \
} while (false)
...
{
    if (options.verbose) {
        logmsg("\t; %016llX %s", op.val, dump_bits(op.val, 64));
    }
    ...
    switch (op.class) {
        ...
        case OPCLASS_BR:    dump_br_op(op.br);
                            break;
        default:
            putmsg("\t%-*s\t%#018llx ; Unhandled Class %d", kMnemonicWidth, ".dq", op.val, op.class);
    }
    return 0;
}
