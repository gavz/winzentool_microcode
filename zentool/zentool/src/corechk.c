...
__declspec(align(256)) cpu_state_t cpu_state;
...
void print_results(const char *name)
{
    ...
    for (uint32_t i = 0; i < 16; i++) {
        printf("  GPR[%d]: 0x%016llx (%s)\n", i, cpu_state.gprs[i], (char *)&cpu_state.gprs[i]);
    }
    printf("  RFLAGS(pre) : 0x%016llx - ", cpu_state.gprs[14]);
    print_rflags(cpu_state.gprs[14]);
    printf("  RFLAGS(post): 0x%016llx - ", cpu_state.gprs[15]);
    print_rflags(cpu_state.gprs[15]);

    for (uint32_t i = 0; i < 16; i++) {
        printf("  YMM[%d]: %08llX:%08llX:%08llX:%08llX\n", i,
