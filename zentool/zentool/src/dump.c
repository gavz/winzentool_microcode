...
const struct ucodeops kNop = {
    .quad[0] = { .value.q = K_NOP_INSN },
    .quad[1] = { .value.q = K_NOP_INSN },
    .quad[2] = { .value.q = K_NOP_INSN },
    .quad[3] = { .value.q = K_NOP_INSN },
...
static void dump_ucode_hdr(const patch_t *patch)
{
    ...
    if (hdr->options.encrypted)
        logmsg("    Use `%s decrypt` to decrypt update", "program.exe");
