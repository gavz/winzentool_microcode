...
#define REGISTER_BITFIELD(_type, _name, _offset, _size) do {                    \
    tableptr->name      = #_name;                               \
    tableptr->position  = _offset;                                \
    tableptr->width     = _size;                                 \
    tableptr++;                                                 \
} while(false);
...
static void init_RegOp_fields(void)
{
    bitfield_t *tableptr = RegOp_fields;

    (void) tableptr;

    #include "gen/RegOp_fields.h"
}

static void init_LdStOp_fields(void)
{
    bitfield_t *tableptr = LdStOp_fields;

    (void) tableptr;

    #include "gen/LdStOp_fields.h"
}

static void init_BrOp_fields(void)
{
    bitfield_t *tableptr = BrOp_fields;

    (void) tableptr;

    #include "gen/BrOp_fields.h"
}

static void init_SRegOp_fields(void)
{
    bitfield_t *tableptr = SRegOp_fields;

    (void) tableptr;

    #include "gen/SRegOp_fields.h"
}
...
bool set_field_name(union BaseOp *op, const char *fieldname, uint64_t value)
{
        ...
        default:
            ...
            if (strcmp(fieldname, "help") == 0) {
                ...
                putmsg("\t%8s (width %2u, position %2u)",
                        "class",
                        /*bitsizeof(union BaseOp, class)*/ 0, // TODO: win port fix
                    /*bitoffsetof(union BaseOp, class)*/ 0);
                return false;
            }
    ...
    dbgmsg("found field %s at position %u with width %u", fieldname, result->position, result->width);
    ...
    if (value > mask) {
        errx(EXIT_FAILURE, "the specified value %#llx is too great for field %s", value, fieldname);
        return false;
    }
